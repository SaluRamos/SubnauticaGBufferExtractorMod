Shader "Hidden/Capture/EmissionMapApproximation"
{
    Properties
    {
        ...
        _AlbedoTex ("Albedo Tex", 2D) = "gray" {}
        _NoLightTex ("NoLight Tex", 2D) = "gray" {}
        _HueDifferenceThreshold ("Hue Difference Threshold", Range(0, 1)) = 0.25
        _MinSaturation ("Min Saturation", Range(0, 1)) = 0.01176
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _NoLightTex;
            float _HueDifferenceThreshold;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                fixed4 noLight = tex2D(_NoLightTex, i.uv);

                float3 mainHSV = rgb2hsv(main.rgb);
                float3 albedoHSV = rgb2hsv(albedo.rgb);
                float3 noLightHSV = rgb2hsv(noLight.rgb);

                float hueDiff = abs(mainHSV.x - albedoHSV.x);
                hueDiff = min(hueDiff, 1.0 - hueDiff);
                if (hueDiff > _HueDifferenceThreshold && mainHSV.y > _MinSaturation)
                {
                    return main;
                }
                return noLight;
